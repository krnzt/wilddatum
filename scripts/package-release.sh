...
base="wilddatum-v${version}-${target_slug}"
mkdir -p "$stage_dir/$base/bin" "$stage_dir/$base/share/wilddatum" "$output_dir"
cp "$binary_path" "$stage_dir/$base/bin/wilddatum"
cp "$repo_dir/LICENSE" "$stage_dir/$base/LICENSE"
cp "$repo_dir/THIRD_PARTY_NOTICES.md" "$stage_dir/$base/THIRD_PARTY_NOTICES.md"
cp -R "$repo_dir/viewer/web-bootstrap/dist" "$stage_dir/$base/share/wilddatum/web"
