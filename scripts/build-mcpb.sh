...
binary_path="${1:-$repo_dir/target/release/wilddatum}"
output_path="${2:-$repo_dir/dist/wilddatum.mcpb}"
platform="${3:-}"
stage_dir="$(mktemp -d)"
...
mkdir -p "$stage_dir/server" "$stage_dir/share/wilddatum"
...
cp "$binary_path" "$stage_dir/server/wilddatum"
cp -R "$repo_dir/viewer/web-bootstrap/dist" "$stage_dir/share/wilddatum/web"
