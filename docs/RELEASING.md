...
EcoScope releases are built from a version tag. GitHub-hosted runners compile
...
- a self-contained `.tar.gz` containing `bin/ecoscope` and the Rerun web bundle;
...
`io.github.krnzt/ecoscope` to the official MCP Registry. No long-lived registry
...
4. Create and push the matching annotated tag, for example:
   ...
   ```bash
   git tag -a v0.1.0-alpha.1 -m "EcoScope v0.1.0-alpha.1"
   git push origin v0.1.0-alpha.1
   ```

5. Verify the release assets on a clean supported machine and confirm the
   registry response for `io.github.krnzt/ecoscope`.
