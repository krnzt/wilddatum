...
pub fn validate_manifest(manifest: &ProviderManifest) -> Result<()> {
    if manifest.schema_version != PROVIDER_PROTOCOL_VERSION {
        return Err(wilddatum_core::WildDatumError::Invalid(format!(
    ...
    {
        return Err(wilddatum_core::WildDatumError::Invalid(
...
pub trait EcologicalDataProvider: Send + Sync {
    ...
    async fn materialize_controlled(
        ...
        if should_cancel() {
            return Err(wilddatum_core::WildDatumError::Conflict(
...
#[cfg(test)]
mod tests {
    use wilddatum_core::{ProviderCapability, ProviderStatus};
