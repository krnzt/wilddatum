...
/// The Rerun release against which WildDatum's semantic event mappings are tested.
...
pub enum DatasetQuery {
    ...
    Spectrum {
        x: u64,
        y: u64,
        /// Explicit rank-3 reflectance dataset. If omitted, WildDatum only
...
pub type Result<T> = std::result::Result<T, WildDatumError>;
