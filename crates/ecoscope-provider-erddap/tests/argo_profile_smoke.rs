...
use ecoscope_core::{
    DatasetRequest, EcoScopeError, ProfileTrajectoryRecipeV1, ProfileValueSpec, ProviderKind,
    ResourceRecord, Result, VerticalAxisSpec, VerticalDirection,
};
use ecoscope_provider_api::EcologicalDataProvider;
use ecoscope_provider_erddap::{ErddapProvider, config};
...
fn variables(resource: &ResourceRecord) -> Result<&Map<String, Value>> {
    resource
        ...
        .ok_or_else(|| EcoScopeError::Invalid("ERDDAP resource omitted variable metadata".into()))
}
...
) -> Result<ecoscope_core::DatasetManifest> {
...
fn materialized_path(
    ...
    manifest: &ecoscope_core::DatasetManifest,
) -> Result<std::path::PathBuf> {
    let object = manifest
        .source_files
        .first()
        .and_then(|source| source.local_object.as_deref())
        .ok_or_else(|| EcoScopeError::Invalid("materialized asset has no local object".into()))?;
...
fn first_profile_key(path: &Path, fields: &ProfileFields) -> Result<ProfileKey> {
    ...
    let trajectory = headers
        ...
        .ok_or_else(|| EcoScopeError::Invalid("discovery omitted trajectory field".into()))?;
    let profile = headers
        .iter()
        .position(|name| name == fields.profile)
        .ok_or_else(|| EcoScopeError::Invalid("discovery omitted profile field".into()))?;
...
fn assert_bounded(manifest: &ecoscope_core::DatasetManifest) -> Result<()> {
    let bytes = manifest
        .source_files
        .iter()
...
fn assert_cf_metadata(
    ...
    let variables = manifest
        ...
        .and_then(Value::as_object)
        .ok_or_else(|| {
            EcoScopeError::Invalid("manifest omitted ERDDAP variable metadata".into())
        })?;
...
fn csv_error(error: csv::Error) -> EcoScopeError {
    EcoScopeError::Invalid(format!("cannot read ERDDAP CSV: {error}"))
}
