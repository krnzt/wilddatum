...
use chrono::Utc;
use ecoscope_core::{DatasetRequest, EcoScopeError, ProviderKind, ResourceQuery, Result};
...
fn live_provider(provider_id: &str) -> Result<ErddapProvider> {
    let preset =
        config::preset(provider_id).ok_or_else(|| EcoScopeError::NotFound(provider_id.into()))?;
    Ok(ErddapProvider::new(preset.into())?.with_object_dir(
        std::env::temp_dir()
            .join("ecoscope-live-erddap")
            .join(provider_id),
    ))
}
