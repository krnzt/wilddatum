...
const SYNTHETIC_LAS: &str = "wilddatum-canopy.las";
const SYNTHETIC_HDF5: &str = "wilddatum-reflectance.h5";
const PROFILE_TRAJECTORY_CSV: &str = "wilddatum-profile-trajectory.csv";
const PROFILE_TRAJECTORY_FIXTURE: &str =
    include_str!("../../wilddatum-rerun/tests/fixtures/profile_trajectory.csv");
...
pub async fn synthetic(service: &WildDatumService) -> Result<DemoResult> {
    ...
    build_demo(
        ...
        DemoDefinition {
            kind: "synthetic",
            name: "WildDatum synthetic canopy and reflectance",
            point_path: &point_path,
            cube_path: &cube_path,
            cube_array: "/WildDatum/Reflectance",
            wavelength_dataset: Some("/WildDatum/Wavelength"),
            rgb: [5, 3, 1],
        },
    )
    .await
...
pub async fn profile_trajectory(service: &WildDatumService) -> Result<DemoResult> {
    ...
    let view = service.create_view(
        "WildDatum synthetic profile and trajectory".into(),
...
async fn build_demo(
    ...
    wilddatum_rerun::write_recording(service, &configured.view_id.0, &recording)?;
    Ok(DemoResult {
...
async fn import_or_reuse(service: &WildDatumService, path: &Path) -> Result<DatasetManifest> {
    let filename = path
        .file_name()
...
fn write_canopy_las(path: &Path) -> Result<()> {
    ...
    builder.system_identifier = "WildDatum synthetic demo".into();
    builder.generating_software = format!("WildDatum {}", env!("CARGO_PKG_VERSION"));
...
fn write_reflectance_cube(path: &Path) -> Result<()> {
    ...
    let group = file.create_group("WildDatum")?;
    group
        .new_dataset_builder()
        .deflate(3)
        .with_data(&values)
...
mod tests {
    ...
    async fn synthetic_demo_uses_real_multimodal_adapters() {
        ...
        assert!(manifests.iter().any(|manifest| {
            manifest
                .modalities
                .contains(&wilddatum_core::Modality::PointCloud)
        }));
        assert!(manifests.iter().any(|manifest| {
            manifest
                .cubes
                .iter()
                .any(|cube| cube.array_path == "/WildDatum/Reflectance")
        }));
    }
