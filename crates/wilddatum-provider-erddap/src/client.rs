...
use url::{Host, Url};
use wilddatum_core::{Result, WildDatumError};
...
impl ErddapClient {
    pub fn new(base_url: &str) -> Result<Self> {
        let base_url = Url::parse(base_url)
            .map_err(|error| WildDatumError::Invalid(format!("invalid ERDDAP URL: {error}")))?;
    ...
    pub async fn server_version(&self) -> Result<String> {
        ...
        let response =
            self.client.get(url).send().await.map_err(|error| {
                WildDatumError::Internal(format!("ERDDAP request failed: {error}"))
            })?;
    ...
    pub async fn resolve_download_chain(&self, url: &Url) -> Result<Vec<Url>> {
        ...
        for _ in 0..5 {
            ...
            let response = self
                ...
                .await
                .map_err(|error| {
                    WildDatumError::Internal(format!("ERDDAP subset probe failed: {error}"))
                })?;
        ...
        F: Fn() -> bool + Send + Sync,
    {
        ...
        let initial = redirect_chain
            .first()
            .ok_or_else(|| WildDatumError::Invalid("ERDDAP redirect chain is empty".into()))?;
        ...
        let response = successful_response.ok_or_else(|| {
            WildDatumError::Conflict("ERDDAP download did not reach an approved endpoint".into())
        })?;
        if response
            .content_length()
        ...
        let result = async {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                size_bytes = size_bytes.checked_add(chunk.len() as u64).ok_or_else(|| {
                    WildDatumError::Invalid("ERDDAP download size overflow".into())
                })?;
    ...
    fn endpoint(&self, segments: &[&str]) -> Result<Url> {
        ...
        let mut path = url
            .path_segments_mut()
            .map_err(|_| WildDatumError::Invalid("ERDDAP base URL cannot be a base".into()))?;
        path.pop_if_empty();
    ...
    async fn get_json(&self, url: Url) -> Result<Value> {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(WildDatumError::NotFound(url.to_string()));
        }
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|error| {
                WildDatumError::Internal(format!("ERDDAP response failed: {error}"))
            })?;
            if body
                .len()
        ...
        serde_json::from_slice(&body).map_err(WildDatumError::from)
    }
}
...
fn redirect_location(current: &Url, headers: &reqwest::header::HeaderMap) -> Result<Url> {
    let location = headers
        ...
        .and_then(|value| value.to_str().ok())
        .ok_or_else(|| WildDatumError::Invalid("ERDDAP redirect omitted Location".into()))?;
    current
        .join(location)
        .map_err(|error| WildDatumError::Invalid(format!("invalid ERDDAP redirect: {error}")))
}
