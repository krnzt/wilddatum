            ...
            for slice in &array.slices {
                ...
                if slice.by_value {
                    ...
                    let start = slice.start.parse::<u64>().map_err(|_| {
                        WildDatumError::Invalid("griddap index start must be an integer".into())
                    })?;
                    let stop = slice.stop.parse::<u64>().map_err(|_| {
                        WildDatumError::Invalid("griddap index stop must be an integer".into())
                    })?;
...
fn constraint_value(constraint: &Constraint) -> Result<String> {
    match &constraint.value {
        ...
        Value::String(value) => serde_json::to_string(value).map_err(WildDatumError::from),
...
fn validate_axis_value(value: &str) -> Result<()> {
    ...
    {
        return Err(WildDatumError::Invalid("invalid ERDDAP axis value".into()));
    }
    Ok(())
}
