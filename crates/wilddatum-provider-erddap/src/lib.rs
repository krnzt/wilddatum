...
use wilddatum_core::{
    ...
    DatasetManifest, DatasetPlan, DatasetRequest, FormatDescriptor, GeoGeometry, LicenseMetadata,
    Modality, PlanId, PlannedFile, ProviderCapability, ProviderKind, ProviderManifest,
    ProviderStatus, ResourceKind, ResourceQuery, ResourceRecord, Result, SourceFile,
    Transformation, WildDatumError,
};
...
impl ErddapProvider {
        ...
        F: Fn() -> bool + Send + Sync,
    {
        ...
        let url = url::Url::parse(download_url)
            .map_err(|error| WildDatumError::Invalid(format!("invalid download URL: {error}")))?;
        ...
        let object_dir = self.object_dir.as_ref().ok_or_else(|| {
            WildDatumError::Internal("ERDDAP object directory was not configured".into())
        })?;
        ...
        if let Err(error) = stored {
            ...
            return Err(WildDatumError::Io(error));
        }
        ...
        P: Fn(usize, usize) + Send + Sync,
    {
        ...
        let search_record = self
            ...
            .ok_or_else(|| WildDatumError::NotFound(plan.request.resource_id.clone()))?;
...
impl EcologicalDataProvider for ErddapProvider {
    ...
    async fn resolve_resource(&self, id: &str) -> Result<ResourceRecord> {
        let record = self
            ...
            .ok_or_else(|| WildDatumError::NotFound(id.into()))?;
    ...
    async fn plan_dataset(&self, request: DatasetRequest) -> Result<DatasetPlan> {
        ...
        .map_err(|error| WildDatumError::Invalid(format!("invalid ERDDAP options: {error}")))?;
        ...
        let search_record = self
            ...
            .ok_or_else(|| WildDatumError::NotFound(request.resource_id.clone()))?;
        ...
        if request.release.is_some() {
            warnings.push("ERDDAP does not apply WildDatum's release field to subset URLs".into());
        }
        DatasetPlan {
            plan_id: PlanId::new(),
            request,
...
fn planned_redirect_chain(file: &PlannedFile, initial: &url::Url) -> Result<Vec<url::Url>> {
    ...
    let values = values
        .as_array()
        .ok_or_else(|| WildDatumError::Invalid("ERDDAP redirect_chain must be an array".into()))?;
    ...
    let chain = values
        ...
        .map(|value| {
            let value = value.as_str().ok_or_else(|| {
                WildDatumError::Invalid("ERDDAP redirect_chain values must be URLs".into())
            })?;
            url::Url::parse(value).map_err(|error| {
                WildDatumError::Invalid(format!("invalid ERDDAP redirect URL: {error}"))
            })
        })
