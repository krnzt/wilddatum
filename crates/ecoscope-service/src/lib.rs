...
use ecoscope_core::{
    CitationMetadata, DatasetId, DatasetManifest, DatasetPlan, EcoLayer, EcoScopeError,
    EcoViewSpec, JobId, JobRecord, JobStatus, LicenseMetadata, LinkedGroup, LocalAssetInspection,
...
impl ServicePaths {
    pub fn discover() -> Result<Self> {
        if let Some(data_dir) = std::env::var_os("ECOSCOPE_DATA_DIR") {
            let data_dir = PathBuf::from(data_dir);
            let cache_dir = std::env::var_os("ECOSCOPE_CACHE_DIR")
                .map(PathBuf::from)
                .unwrap_or_else(|| data_dir.join("cache"));
            return Ok(Self::under(data_dir, cache_dir));
        }
        let dirs = ProjectDirs::from("org", "EcoScope", "EcoScope").ok_or_else(|| {
            EcoScopeError::Internal("cannot determine user data directory".into())
        })?;
...
impl EcoScopeService {
    ...
    fn connection(&self) -> Result<MutexGuard<'_, Connection>> {
        self.connection
            .lock()
            .map_err(|_| EcoScopeError::Internal("state database lock was poisoned".into()))
    }
    ...
    pub fn enrich_manifest_metadata(&self, manifest: &mut DatasetManifest) {
        for source in &mut manifest.source_files {
            ...
            match ecoscope_local_import::inspect_hdf5_structure(&path) {
                Ok(structure) => {
                    source
                        .metadata
    ...
    pub async fn import_local_file(&self, path: &Path) -> Result<DatasetManifest> {
        let inspection = ecoscope_local_import::inspect_path(path).await?;
        ...
        let manifest = DatasetManifest {
            ...
            transformations: vec![],
            format: Some(ecoscope_core::FormatDescriptor {
            ...
            spatial_reference: inspection.crs.as_ref().map(|crs| {
                ...
                ecoscope_core::SpatialReference {
                    authority,
                    code,
                    wkt: None,
                    axis_order: vec![],
    ...
    pub fn inspect_asset(&self, asset_id: &str) -> Result<LocalAssetInspection> {
        let text = self
            ...
            .optional()
            .map_err(db_error)?
            .ok_or_else(|| EcoScopeError::NotFound(format!("asset {asset_id}")))?;
    ...
    pub fn validate_local_asset(&self, asset_id: &str) -> Result<bool> {
        let (path, expected): (String, String) = self
            ...
            .map_err(db_error)?
            .ok_or_else(|| EcoScopeError::NotFound(format!("asset {asset_id}")))?;
        let fingerprint = ecoscope_local_import::fingerprint_path(Path::new(&path))?;
    ...
    pub fn preview_dataset(&self, dataset_id: &str, limit: u32) -> Result<Value> {
        ...
        let mut reader = csv::ReaderBuilder::new()
            .delimiter(delimiter)
            .from_path(&path)
            .map_err(|error| EcoScopeError::Invalid(error.to_string()))?;
        let headers = reader
            .headers()
            .map_err(|error| EcoScopeError::Invalid(error.to_string()))?
            .iter()
        ...
        for record in reader.records().take(limit.min(2000) as usize) {
            let record = record.map_err(|error| EcoScopeError::Invalid(error.to_string()))?;
            rows.push(
                headers
                    .iter()
                    .zip(record.iter())
    ...
    pub fn create_view(&self, name: String, dataset_ids: Vec<DatasetId>) -> Result<EcoViewSpec> {
        ...
        for (index, dataset_id) in dataset_ids.iter().enumerate() {
                ...
                && manifest.cubes[0].axes.len() == 3
            {
                ...
                let axis_for = |role: ecoscope_core::AxisRole| {
                    descriptor.axes.iter().position(|axis| axis.role == role)
                };
                if let (Some(y_axis), Some(x_axis), Some(spectral_axis)) = (
                    axis_for(ecoscope_core::AxisRole::Y),
                    axis_for(ecoscope_core::AxisRole::X),
                    axis_for(ecoscope_core::AxisRole::Spectral),
                ) {
    ...
    pub fn patch_view(
        ...
        let updated: EcoViewSpec = serde_json::from_value(value).map_err(|error| {
            EcoScopeError::Invalid(format!("view patch does not produce a valid spec: {error}"))
        })?;
    ...
    pub fn latest_selection(&self, view_id: &str) -> Result<SelectionRecord> {
        let text = self
            ...
            .optional()
            .map_err(db_error)?
            .ok_or_else(|| EcoScopeError::NotFound(format!("selection for view {view_id}")))?;
        serde_json::from_str(&text).map_err(EcoScopeError::from)
    }
    ...
    fn get_json<T: DeserializeOwned>(&self, table: &str, id: &str) -> Result<T> {
        ...
        let text = self
            ...
            .optional()
            .map_err(db_error)?
            .ok_or_else(|| EcoScopeError::NotFound(format!("{table} {id}")))?;
        serde_json::from_str(&text).map_err(EcoScopeError::from)
    }
...
fn db_error(error: rusqlite::Error) -> EcoScopeError {
    EcoScopeError::Internal(format!("state database error: {error}"))
}
