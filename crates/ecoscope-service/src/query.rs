...
use chrono::Utc;
use ecoscope_core::{
    ArtifactDescriptor, DatasetQuery, EcoScopeError, ExportFormat, ExportRecord, ExportRequest,
    Result, ResultId, ResultRecord,
};
...
impl EcoScopeService {
    ...
    pub async fn query_dataset(
        ...
        let (payload, row_count, media_type) = match &query {
            ...
            DatasetQuery::Table {
                ...
                let output = ecoscope_query::execute_table_query(
                    &path,
                    &source.original_name,
                    ecoscope_query::TableQuerySpec {
                        select,
                        filters,
                        group_by,
                        aggregates,
        ...
        self.connection()?
            .execute(
                ...
                rusqlite::params![
                    ...
                    serde_json::to_string(&json!({"engine": "ecoscope-query"}))?,
    ...
    fn persist_result(
        ...
        let checksum = ecoscope_local_import::fingerprint_path(&path)?;
        let mut artifacts = vec![ArtifactDescriptor {
            uri: format!("ecoscope://results/{}/data", result_id.0),
            format: "json".into(),
        ...
        if let Some(batches) = table_batches.filter(|batches| !batches.is_empty()) {
            ...
            ecoscope_query::write_arrow_ipc(&arrow_path, batches)?;
            let arrow_checksum = ecoscope_local_import::fingerprint_path(&arrow_path)?;
            artifacts.push(ArtifactDescriptor {
                uri: format!("ecoscope://results/{}/arrow", result_id.0),
        ...
        let record = ResultRecord {
            ...
            dataset_id: ecoscope_core::DatasetId(dataset_id.to_owned()),
            source_selection: None,
            query,
            row_count,
            preview,
            artifact: Some(format!("ecoscope://results/{}/data", result_id.0)),
    ...
    pub fn export_result(&self, request: ExportRequest) -> Result<ExportRecord> {
                ...
                let crate_document = json!({
                    ...
                    "@graph": [
                        ...
                        },
                        {
                            ...
                            "ecoscope:query": result.query,
                            "ecoscope:result": payload
                        }
                    ]
                });
        ...
        let manifest_artifact = if request.include_provenance {
            ...
            Some(format!("ecoscope://exports/{export_id}/manifest"))
        } else {
            None
        };
        let record = ExportRecord {
            ...
            format: request.format,
            artifact: format!("ecoscope://exports/{export_id}/data"),
            checksum,
            manifest_artifact,
            created_at: Utc::now(),
        };
    ...
    fn write_geotiff_export(
        ...
        let bounds = payload
            ...
            .ok_or_else(|| EcoScopeError::Invalid("raster result has no pixel bounds".into()))?;
        ...
        let raster = geotiff_reader::GeoTiffFile::open(&source_path)
            .map_err(|error| EcoScopeError::Invalid(format!("cannot open GeoTIFF: {error}")))?;
        ...
        let data = ndarray::Array3::from_shape_vec((rows, cols, selected.len()), interleaved)
            .map_err(|error| {
                EcoScopeError::Internal(format!("cannot shape raster export: {error}"))
            })?;
        let source_transform = raster.transform().ok_or_else(|| {
            EcoScopeError::Invalid("source GeoTIFF has no affine transform".into())
        })?;
        ...
        geotiff_writer::CogBuilder::new(builder)
            ...
            .map_err(|error| EcoScopeError::Internal(format!("cannot write COG export: {error}")))
    }
}
...
fn query_spectrum(
    ...
    let file = hdf5_metno::File::open(path)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot open HDF5: {error}")))?;
...
fn read_hdf5_spectrum(dataset: &hdf5_metno::Dataset, y: usize, x: usize) -> Result<Vec<f64>> {
    let datatype = dataset
        .dtype()
        .map_err(|error| EcoScopeError::Invalid(format!("cannot inspect HDF5 type: {error}")))?;
    ...
    values.map_err(|error| EcoScopeError::Invalid(format!("cannot read spectrum: {error}")))
}

fn read_hdf5_vector(file: &hdf5_metno::File, path: &str) -> Result<Vec<f64>> {
    let dataset = file.dataset(path).map_err(|error| {
        EcoScopeError::Invalid(format!("cannot open wavelength dataset {path}: {error}"))
    })?;
    ...
    let datatype = dataset
        .dtype()
        .map_err(|error| EcoScopeError::Invalid(format!("cannot inspect wavelengths: {error}")))?;
    macro_rules! read_values {
        ($type:ty) => {
            dataset
                .read_raw::<$type>()
    ...
    values.map_err(|error| EcoScopeError::Invalid(format!("cannot read wavelengths: {error}")))
}
...
fn query_point_cloud(
    ...
    let [min_x, min_y, max_x, max_y] = geometry_bounds(geometry).ok_or_else(|| {
        EcoScopeError::Invalid("point cloud geometry must contain valid GeoJSON coordinates".into())
    })?;
    ...
    for point in reader.points() {
        let point = point
            .map_err(|error| EcoScopeError::Invalid(format!("cannot read LAS point: {error}")))?;
...
fn query_copc_point_cloud(
    ...
    let mut reader = copc_rs::CopcReader::from_path(path)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot open COPC: {error}")))?;
    ...
    let mut points = reader
        ...
        .map_err(|error| EcoScopeError::Invalid(format!("cannot query COPC octree: {error}")))?;
    let limit = requested_limit.clamp(1, MAX_POINT_RESULT_ROWS as u64) as usize;
    let classes = classifications.iter().copied().collect::<BTreeSet<_>>();
...
fn query_raster_pixel(
    ...
    let image = image::open(path)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot decode raster: {error}")))?;
    ...
    for band in chosen {
        let sample =
            read_geotiff_band_window(&raster, band as usize, y as usize, x as usize, 1, 1)?
                .into_iter()
                .next()
                .ok_or_else(|| EcoScopeError::Internal("GeoTIFF pixel window was empty".into()))?;
...
fn query_raster_region(
    ...
    let [min_x, min_y, max_x, max_y] = geometry_bounds(geometry).ok_or_else(|| {
        EcoScopeError::Invalid("raster geometry must contain valid GeoJSON coordinates".into())
    })?;
    let corners = [
        (min_x, min_y),
        (min_x, max_y),
    ...
    let pixels = corners
        ...
        .map(|(world_x, world_y)| {
            raster.geo_to_pixel(*world_x, *world_y).ok_or_else(|| {
                EcoScopeError::Invalid("GeoTIFF transform is singular or unavailable".into())
            })
        })
        .collect::<Result<Vec<_>>>()?;
...
fn query_vector_region(
    ...
    let query_geometry: geo::Geometry<f64> = (&query_geometry.value)
        .try_into()
        .map_err(|error| EcoScopeError::Invalid(format!("invalid query geometry: {error}")))?;
    let document: geojson::GeoJson = serde_json::from_reader(File::open(path)?)
        .map_err(|error| EcoScopeError::Invalid(format!("invalid GeoJSON source: {error}")))?;
...
async fn query_vector_region_dispatch(
    ...
    if matches!(extension.as_str(), "parquet" | "geoparquet") {
        ...
        let query_geometry: geo::Geometry<f64> = (&query_geometry.value)
            .try_into()
            .map_err(|error| EcoScopeError::Invalid(format!("invalid query geometry: {error}")))?;
        let output = ecoscope_query::execute_geoparquet_region(
            path,
...
fn geoparquet_row_to_feature(index: usize, row: Value) -> Result<Value> {
    let mut properties = row
        .as_object()
        .cloned()
        .ok_or_else(|| EcoScopeError::Internal("GeoParquet result row is not an object".into()))?;
    let geometry_text = properties
        .remove("__ecoscope_geometry_wkt")
        .and_then(|value| value.as_str().map(str::to_owned))
        .ok_or_else(|| EcoScopeError::Internal("GeoParquet row has no decoded geometry".into()))?;
    let parsed: wkt::Wkt<f64> = geometry_text
        .parse()
        .map_err(|error| EcoScopeError::Invalid(format!("invalid GeoParquet WKT: {error}")))?;
    let geometry: geo::Geometry<f64> = parsed.try_into().map_err(|error| {
        EcoScopeError::Invalid(format!("unsupported GeoParquet geometry: {error}"))
    })?;
...
fn query_flatgeobuf_region(
    ...
    const MAX_VECTOR_RESULTS: usize = 100_000;
    let [min_x, min_y, max_x, max_y] = geometry_bounds(geometry).ok_or_else(|| {
        EcoScopeError::Invalid("vector geometry must contain valid GeoJSON coordinates".into())
    })?;
    let query_geometry = geojson_geometry(geometry)?;
    let query_geometry: geo::Geometry<f64> = (&query_geometry.value)
        .try_into()
    ...
    let reader = flatgeobuf::FgbReader::open(input)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot open FlatGeobuf: {error}")))?;
    let mut features = reader
        .select_bbox(min_x, min_y, max_x, max_y)
        .map_err(|error| {
            EcoScopeError::Invalid(format!("cannot query FlatGeobuf index: {error}"))
        })?;
    let mut rows = Vec::new();
    let mut matched = 0_u64;
    let mut index = 0_u64;
    while let Some(feature) = features
        .next()
        .map_err(|error| EcoScopeError::Invalid(format!("cannot read FlatGeobuf: {error}")))?
    {
        let text = feature.to_json().map_err(|error| {
            EcoScopeError::Invalid(format!("cannot decode FlatGeobuf feature: {error}"))
        })?;
        let feature: geojson::Feature = text.parse().map_err(|error| {
            EcoScopeError::Invalid(format!("invalid FlatGeobuf geometry: {error}"))
        })?;
        let intersects = feature.geometry.as_ref().is_some_and(|geometry| {
            let candidate: std::result::Result<geo::Geometry<f64>, _> =
                (&geometry.value).try_into();
...
fn query_shapefile_region(
    ...
    let mut reader = shapefile::Reader::from_path(path)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot open Shapefile: {error}")))?;
    let mut rows = Vec::new();
    let mut matched = 0_u64;
    for (index, entry) in reader.iter_shapes_and_records().enumerate() {
        let (shape, record) = entry
            .map_err(|error| EcoScopeError::Invalid(format!("cannot read Shapefile: {error}")))?;
...
fn geojson_geometry(value: &Value) -> Result<geojson::Geometry> {
    ...
    serde_json::from_value(geometry)
        .map_err(|error| EcoScopeError::Invalid(format!("invalid GeoJSON geometry: {error}")))
}
    ...
    let columns = payload
        ...
        .map(|value| {
            value
                .as_str()
                .map(str::to_owned)
                .ok_or_else(|| EcoScopeError::Invalid("invalid result column".into()))
        })
        .collect::<Result<Vec<_>>>()?;
    let rows = payload
        .get("rows")
...
fn write_csv_export(path: &Path, payload: &Value) -> Result<()> {
    ...
    let mut writer = csv::Writer::from_path(path)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot create CSV export: {error}")))?;
    writer
        .write_record(&columns)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot write CSV header: {error}")))?;
    for row in rows {
        writer
            .write_record(
                columns
            ...
            .map_err(|error| EcoScopeError::Invalid(format!("cannot write CSV row: {error}")))?;
    }
    writer
        .flush()
        .map_err(|error| EcoScopeError::Invalid(format!("cannot finish CSV export: {error}")))
}
...
fn write_parquet_export(path: &Path, payload: &Value) -> Result<()> {
    ...
    let batch = RecordBatch::try_new(schema.clone(), arrays)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot build Arrow batch: {error}")))?;
    let mut writer = ArrowWriter::try_new(File::create(path)?, schema, None).map_err(|error| {
        EcoScopeError::Internal(format!("cannot create Parquet export: {error}"))
    })?;
    writer.write(&batch).map_err(|error| {
        EcoScopeError::Internal(format!("cannot write Parquet export: {error}"))
    })?;
    writer.close().map_err(|error| {
        EcoScopeError::Internal(format!("cannot finish Parquet export: {error}"))
    })?;
    Ok(())
}
...
fn write_geojson_export(path: &Path, payload: &Value) -> Result<()> {
    let rows = payload
        .get("rows")
        .and_then(Value::as_array)
        .ok_or_else(|| EcoScopeError::Invalid("result is not a vector feature result".into()))?;
    let features = rows
        .iter()
        .map(|row| {
            let geometry = row
                ...
                .transpose()
                .map_err(|error| {
                    EcoScopeError::Invalid(format!("invalid result geometry: {error}"))
                })?;
...
mod tests {
    ...
    async fn queries_general_hdf5_zarr_and_netcdf3_cube_slices() {
        ...
        assert_eq!(zarr.cubes[0].axes[0].role, ecoscope_core::AxisRole::Time);
