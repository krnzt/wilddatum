...
use wilddatum_core::{
    ...
    DatasetManifest, DatasetPlan, DatasetRequest, LicenseMetadata, Modality, PlannedFile,
    ProviderCapability, ProviderKind, ProviderManifest, ProviderStatus, ResourceKind,
    ResourceRecord, Result, SourceFile, WildDatumError,
};
use wilddatum_provider_api::{EcologicalDataProvider, PROVIDER_PROTOCOL_VERSION};
...
impl NeonProvider {
    pub fn new(token: Option<String>) -> Result<Self> {
        let client = Client::builder()
            .user_agent(concat!("wilddatum/", env!("CARGO_PKG_VERSION")))
            .build()
            .map_err(|error| WildDatumError::Internal(error.to_string()))?;
        Ok(Self {
            client,
    ...
    async fn json(&self, request: reqwest::RequestBuilder) -> Result<Value> {
        let response =
            self.authenticated(request).send().await.map_err(|error| {
                WildDatumError::Internal(format!("NEON request failed: {error}"))
            })?;
        ...
        if status == StatusCode::UNAUTHORIZED || status == StatusCode::FORBIDDEN {
            return Err(WildDatumError::CredentialsRequired("neon".into()));
        }
        if !status.is_success() {
            return Err(WildDatumError::Internal(format!(
    ...
    async fn product_value(&self, product_code: &str) -> Result<Value> {
        ...
        value
            .get("data")
            .cloned()
            .ok_or_else(|| WildDatumError::NotFound(product_code.into()))
    }
    ...
    async fn exact_plan(&self, request: &DatasetRequest) -> Result<Vec<PlannedFile>> {
        ...
        let start = request
            .temporal_start
            .as_deref()
            .ok_or_else(|| WildDatumError::Invalid("temporal_start is required for NEON".into()))?;
        let end = request
            .temporal_end
            .as_deref()
            .ok_or_else(|| WildDatumError::Invalid("temporal_end is required for NEON".into()))?;
    ...
    {
        if should_cancel() {
            return Err(WildDatumError::Conflict("materialization cancelled".into()));
        }
        let url = file
            .download_url
            .as_deref()
            .ok_or_else(|| WildDatumError::Invalid(format!("{} has no download URL", file.name)))?;
        let response = self
            .authenticated(self.client.get(url))
            .send()
            .await
            .map_err(|error| WildDatumError::Internal(format!("download failed: {error}")))?;
        ...
        let object_dir = self.object_dir.as_ref().ok_or_else(|| {
            WildDatumError::Internal("NEON object directory was not configured".into())
        })?;
...
impl EcologicalDataProvider for NeonProvider {
    ...
    async fn search_catalog(&self, query: CatalogQuery) -> Result<Vec<CatalogEntry>> {
        ...
        let products = value
            ...
            .and_then(Value::as_array)
            .ok_or_else(|| WildDatumError::Internal("unexpected NEON products response".into()))?;
    ...
    async fn inspect_product(&self, id: &str) -> Result<CatalogEntry> {
        ...
        Self::product_entry(&value).ok_or_else(|| WildDatumError::NotFound(id.into()))
    }

    async fn resolve_resource(&self, id: &str) -> Result<ResourceRecord> {
        let raw = self.product_value(id).await?;
        let entry = Self::product_entry(&raw).ok_or_else(|| WildDatumError::NotFound(id.into()))?;
    ...
    async fn plan_dataset(&self, request: DatasetRequest) -> Result<DatasetPlan> {
        ...
        DatasetPlan {
            plan_id: wilddatum_core::PlanId::new(),
            request,
