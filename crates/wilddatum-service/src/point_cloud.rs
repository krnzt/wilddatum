...
use wilddatum_core::{ArtifactDescriptor, DatasetId, DerivedAssetRecord, Result, WildDatumError};
...
impl WildDatumService {
    ...
    pub fn derive_copc_index(&self, dataset_id: &str) -> Result<DerivedAssetRecord> {
        ...
        let mut writer = copc_rs::CopcWriter::from_path(&temporary_path, header, 256, 16_384)
            .map_err(|error| {
                WildDatumError::Internal(format!("cannot create COPC index: {error}"))
            })?;
        writer.write(points, point_count).map_err(|error| {
            WildDatumError::Invalid(format!("cannot derive COPC index: {error}"))
        })?;
        drop(writer);
        ...
        temporary
            ...
            .map_err(|error| WildDatumError::Io(error.error))?;
        ...
        let record = DerivedAssetRecord {
            ...
            artifact: ArtifactDescriptor {
                uri: format!("wilddatum://derived/{derived_id}/data"),
                format: "copc".into(),
            ...
            metadata: [
                ...
                (
                    "warning".into(),
                    json!("The copc-rs 0.5 writer does not guarantee representative lower LOD point distributions; WildDatum will not use resolution/level queries on this derived index."),
                ),
            ]
            .into_iter()
            .collect(),
    ...
    pub fn find_derived_asset(
        ...
        let text = self
            ...
            .optional()
            .map_err(|error| WildDatumError::Internal(format!("database error: {error}")))?;
        text.map(|text| serde_json::from_str(&text).map_err(WildDatumError::from))
            .transpose()
    }
