...
use wilddatum_core::{CubeRange, Result, WildDatumError};
...
fn validate_ranges(shape: &[u64], ranges: &[CubeRange], limit: u64) -> Result<Vec<u64>> {
    ...
    for (axis, (range, length)) in ranges.iter().zip(shape).enumerate() {
        ...
        cells = cells
            .checked_mul(count)
            .ok_or_else(|| WildDatumError::Invalid("cube slice cell count overflows u64".into()))?;
...
fn query_hdf5_slice(
    ...
    .map_err(|error| WildDatumError::Invalid(format!("cannot read HDF5 slice: {error}")))?;
    Ok(cube_payload(
        array_path,
        &shape,
        ranges,
...
fn query_zarr_slice(
    ...
    let array = Array::open(store, array_path).map_err(|error| {
        WildDatumError::Invalid(format!("cannot open Zarr array {array_path}: {error}"))
    })?;
    ...
    let bounding_cells = ranges
        ...
        .ok_or_else(|| WildDatumError::Invalid("Zarr bounding slice size overflows u64".into()))?;
    ...
    .map_err(|error| WildDatumError::Invalid(format!("cannot decode Zarr slice: {error}")))?;
...
fn query_netcdf3_slice(
    ...
    let variable = reader
        .data_set()
        .get_var(array_path)
        .ok_or_else(|| WildDatumError::NotFound(format!("NetCDF-3 variable {array_path}")))?;
    let shape = variable
        .get_dims()
        .iter()
    ...
    if source_cells > MAX_NETCDF3_SOURCE_CELLS {
        return Err(WildDatumError::Invalid(format!(
            "NetCDF-3 variable {array_path} has {source_cells} cells; the pure-Rust reader cannot perform subset I/O yet, so WildDatum limits full-variable reads to {MAX_NETCDF3_SOURCE_CELLS} cells"
        )));
    }
    let output_shape = validate_ranges(&shape, ranges, limit)?;
