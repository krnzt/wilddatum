//! WildDatum MCP server: a scientific interface, not a mirror of provider APIs.
...
use wilddatum_provider_erddap::{ErddapProvider, config as erddap_config};
use wilddatum_provider_neon::NeonProvider;
use wilddatum_provider_process::{ProcessProvider, discover_configs, find_config};
use wilddatum_service::WildDatumService;

const KEYRING_SERVICE: &str = "org.wilddatum.WildDatum";
#[cfg(not(test))]
const LEGACY_KEYRING_SERVICE: &str = "org.ecoscope.EcoScope";
const KEYRING_NEON_USER: &str = "neon-api-token";

pub fn built_in_provider_manifests() -> std::result::Result<Vec<ProviderManifest>, WildDatumError> {
...
) -> std::result::Result<Box<dyn EcologicalDataProvider>, WildDatumError> {
...
#[derive(Clone)]
pub struct WildDatumMcp {
    service: WildDatumService,
    tool_router: ToolRouter<Self>,
}

impl WildDatumMcp {
    pub fn new(service: WildDatumService) -> Self {
        Self {
            service,
...
pub struct SearchCatalogInput {
    ...
    #[serde(default)]
    pub modalities: Vec<wilddatum_core::Modality>,
...
pub struct PlanMaterializationInput {
    ...
    pub spatial_filter: Option<wilddatum_core::GeoGeometry>,
    #[serde(default)]
...
#[tool_router]
impl WildDatumMcp {
    #[tool(description = "Check WildDatum storage, protocol, provider, and viewer readiness")]
    async fn health(&self) -> CallToolResult {
        ...
        health["rerun_version"] = json!(wilddatum_rerun::PINNED_RERUN_VERSION);
    ...
    async fn open_connection_setup(&self) -> CallToolResult {
        bounded_result(json!({
            ...
            "command": "wilddatum connect-neon",
    ...
    async fn plan_materialization(
        ...
        if is_neon && (input.temporal_start.is_none() || input.temporal_end.is_none()) {
            return tool_error(WildDatumError::Invalid(
    ...
    async fn materialize_dataset(
        ...
        let (provider_id, token, credentials) = match &plan.request.provider {
            ProviderKind::Neon => match load_neon_token() {
                ...
                None => {
                    return tool_error(WildDatumError::CredentialsRequired("neon".into()));
                }
            },
        ...
        tokio::spawn(async move {
            ...
            let outcome: Result<_, WildDatumError> = async {
    ...
    #[tool(description = "Inspect progress and results for a durable WildDatum job")]
    ...
    #[tool(description = "Request cancellation of a queued or running WildDatum job")]
    ...
    #[tool(
        description = "Create a bounded, regenerable Rerun recording for a semantic WildDatum view"
    )]
    async fn render_view(&self, Parameters(input): Parameters<ViewIdInput>) -> CallToolResult {
        ...
        match wilddatum_rerun::write_recording(&self.service, &input.view_id, &destination) {
            Ok(_) => bounded_result(json!({
                "view_id": input.view_id,
                "artifact": format!("wilddatum://views/{}/recording", input.view_id),
                "renderer": "rerun",
                "status": "ready"
            })),
            Err(error) => tool_error(error),
    ...
    async fn open_view(&self, Parameters(input): Parameters<ViewIdInput>) -> CallToolResult {
        ...
        let executable = match std::env::current_exe() {
            ...
            Err(error) => return tool_error(WildDatumError::Io(error)),
        };
        let child = std::process::Command::new(executable)
            .args(["serve", &input.view_id, "--port", "0", "--open"])
            .env("WILDDATUM_DATA_DIR", &self.service.paths().data_dir)
            .env("WILDDATUM_CACHE_DIR", &self.service.paths().cache_dir)
        ...
        match child {
            ...
            Err(error) => tool_error(WildDatumError::Internal(format!(
                "could not start browser explorer: {error}"
            ))),
        }
    }

    #[tool(description = "Render and open an WildDatum view in the installed native Rerun viewer")]
    async fn open_native_view(&self, Parameters(input): Parameters<ViewIdInput>) -> CallToolResult {
        let destination = self
            ...
            .join(format!("{}.rrd", input.view_id));
        if let Err(error) =
            wilddatum_rerun::write_recording(&self.service, &input.view_id, &destination)
        {
            return tool_error(error);
        }
        match wilddatum_rerun::open_recording(&destination) {
...
impl ServerHandler for WildDatumMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "WildDatum discovers, plans, materializes, queries, visualizes, selects, and cites ecological data. Always plan and inspect before materializing. Never request credentials or local file paths in conversation.",
        )
    }
}

pub async fn run_stdio(service: WildDatumService) -> anyhow::Result<()> {
    let server = WildDatumMcp::new(service)
...
fn bounded_result(value: Value) -> CallToolResult {
    match serde_json::to_vec(&value) {
        ...
        Ok(bytes) => CallToolResult::error(vec![ContentBlock::text(format!(
            "result is {} bytes, above WildDatum's {} byte MCP limit; narrow the query or use an artifact handle",
            bytes.len(),
...
fn tool_error(error: WildDatumError) -> CallToolResult {
    let code = match error {
        WildDatumError::CredentialsRequired(_) => "NEON_CONNECTION_REQUIRED",
        WildDatumError::NotFound(_) => "NOT_FOUND",
        WildDatumError::Conflict(_) => "CONFLICT",
        WildDatumError::Invalid(_) => "INVALID_REQUEST",
        _ => "WILDDATUM_ERROR",
    };
...
fn load_neon_token() -> Option<String> {
    ...
    [KEYRING_SERVICE, LEGACY_KEYRING_SERVICE]
        .into_iter()
        .find_map(|service| {
            keyring::Entry::new(service, KEYRING_NEON_USER)
                .ok()
...
fn fail_job(service: &WildDatumService, job_id: &str, error: String) {
...
pub fn store_neon_token(token: &str) -> Result<(), WildDatumError> {
    if token.trim().len() < 8 {
    ...
    let entry = keyring::Entry::new(KEYRING_SERVICE, KEYRING_NEON_USER)
        .map_err(|error| WildDatumError::Internal(format!("keychain error: {error}")))?;
    entry
        .set_password(token.trim())
        .map_err(|error| WildDatumError::Internal(format!("keychain error: {error}")))
}

pub fn remove_neon_token() -> Result<(), WildDatumError> {
    let entry = keyring::Entry::new(KEYRING_SERVICE, KEYRING_NEON_USER)
        .map_err(|error| WildDatumError::Internal(format!("keychain error: {error}")))?;
    entry
        .delete_credential()
...
mod tests {
    ...
    async fn registered_client_can_discover_and_call_tools() -> anyhow::Result<()> {
        ...
        let registry = WildDatumMcp::new(service.clone());
        ...
        assert!(
            export
                ...
                .and_then(Value::as_str)
                .is_some_and(|artifact| artifact.starts_with("wilddatum://exports/"))
        );
