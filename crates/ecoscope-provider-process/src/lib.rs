...
use ecoscope_core::{
    ...
    EcoScopeError, ProviderManifest, ResourceQuery, ResourceRecord, Result,
};
...
impl ProcessProvider {
    pub async fn spawn(config: ProcessProviderConfig) -> Result<Self> {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| EcoScopeError::Internal("provider stdin is unavailable".into()))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| EcoScopeError::Internal("provider stdout is unavailable".into()))?;
    ...
    fn validate_plan_origins(&self, plan: &DatasetPlan) -> Result<()> {
        for file in &plan.files {
            ...
            let url = Url::parse(download_url).map_err(|error| {
                EcoScopeError::Invalid(format!("provider returned invalid download URL: {error}"))
            })?;
            let host = url.host_str().ok_or_else(|| {
                EcoScopeError::Invalid("provider download URL has no host".into())
            })?;
...
impl EcologicalDataProvider for ProcessProvider {
    ...
    async fn materialize(
        ...
        if credentials.is_some() || plan.requires_credentials {
            return Err(EcoScopeError::Invalid(
                "community subprocess providers cannot receive credential values; authenticated fetching must be implemented by an EcoScope-owned credential broker"
                    .into(),
            ));
        }
...
async fn rpc_call<T: DeserializeOwned>(
    ...
    let response: RpcResponse = serde_json::from_slice(&line).map_err(|error| {
        EcoScopeError::Invalid(format!("provider returned invalid JSON-RPC: {error}"))
    })?;
    ...
    let result = response
        .result
        .ok_or_else(|| EcoScopeError::Invalid("provider response has no result".into()))?;
