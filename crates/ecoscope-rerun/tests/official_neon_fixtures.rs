...
use ecoscope_core::{DatasetId, DatasetQuery, GeoGeometry};
...
async fn renders_official_neon_point_cloud_and_hyperspectral_subsets() {
    ...
    ecoscope_rerun::write_recording(&service, &point_view.view_id.0, &point_rrd).unwrap();
    ...
    ecoscope_rerun::write_recording(&service, &hyperspectral_view.view_id.0, &hyperspectral_rrd)
        .unwrap();
