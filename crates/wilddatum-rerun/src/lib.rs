//! Stable WildDatum-to-Rerun boundary.
//!
...
pub use wilddatum_core::PINNED_RERUN_VERSION;
...
pub fn write_recording_with_link_resolution(
    ...
    let application_id = rerun::ApplicationId::try_new(format!("wilddatum-{}", view.view_id))
        .map_err(rerun_error)?;
...
fn log_image(recording: &rerun::RecordingStream, entity_root: &str, path: &Path) -> Result<()> {
    let image = image::open(path)
        .map_err(|error| WildDatumError::Invalid(format!("cannot decode image/raster: {error}")))?;
    let image = rerun::Image::from_image(image)
        .map_err(|error| WildDatumError::Invalid(format!("cannot convert image: {error}")))?;
    recording
...
fn log_point_cloud(
    ...
    let mut reader = las::Reader::from_path(path)
        .map_err(|error| WildDatumError::Invalid(format!("cannot decode LAS/LAZ: {error}")))?;
    ...
    for (index, point) in reader.points().enumerate() {
        ...
        let point = point.map_err(|error| {
            WildDatumError::Invalid(format!("cannot decode LAS point: {error}"))
        })?;
        positions.push([
    ...
    match extension {
        ...
        "fgb" => {
            ...
            let mut reader = flatgeobuf::FgbReader::open(input)
                ...
                .map_err(|error| {
                    WildDatumError::Invalid(format!("cannot open FlatGeobuf: {error}"))
                })?;
            let mut output = Vec::new();
            while output.len() < MAX_VECTOR_FEATURES {
                let Some(feature) = reader.next().map_err(|error| {
                    WildDatumError::Invalid(format!("cannot read FlatGeobuf: {error}"))
                })?
                else {
                    break;
                };
                let text = feature.to_json().map_err(|error| {
                    WildDatumError::Invalid(format!("cannot decode FlatGeobuf: {error}"))
                })?;
                output.push(text.parse().map_err(|error| {
                    WildDatumError::Invalid(format!("invalid FlatGeobuf feature: {error}"))
                })?);
            }
            Ok(output)
        }
        "shp" => {
            let mut reader = shapefile::Reader::from_path(path).map_err(|error| {
                WildDatumError::Invalid(format!("cannot open Shapefile: {error}"))
            })?;
            reader
                ...
                .map(|entry| {
                    let (shape, _) = entry.map_err(|error| {
                        WildDatumError::Invalid(format!("cannot read Shapefile: {error}"))
                    })?;
...
fn log_hdf5_hyperspectral(
    ...
    let file = hdf5_metno::File::open(path)
        .map_err(|error| WildDatumError::Invalid(format!("cannot open HDF5/NetCDF-4: {error}")))?;
...
fn log_zarr_hyperspectral(
    ...
    let array = Array::open(store, dataset_path).map_err(|error| {
        WildDatumError::Invalid(format!("cannot open Zarr array {dataset_path}: {error}"))
    })?;
    let shape = array
        .shape()
        .iter()
...
fn log_netcdf3_hyperspectral(
    ...
    let variable = reader
        ...
        .ok_or_else(|| WildDatumError::NotFound(format!("NetCDF-3 variable {dataset_path}")))?;
    let shape = variable
        .get_dims()
        .iter()
    ...
    let values = match reader.read_var(dataset_path).map_err(|error| {
        WildDatumError::Invalid(format!("cannot read NetCDF-3 variable: {error}"))
    })? {
...
fn read_hdf5_band(
    ...
    let datatype = dataset.dtype().map_err(|error| {
        WildDatumError::Invalid(format!("cannot inspect HDF5 datatype: {error}"))
    })?;
    ...
    values.map_err(|error| WildDatumError::Invalid(format!("cannot read HDF5 band: {error}")))?
}
...
fn read_zarr_band<TStorage: ?Sized + zarrs::storage::ReadableStorageTraits + 'static>(
    ...
    .map_err(|error| WildDatumError::Invalid(format!("cannot read Zarr band: {error}")))?;
...
fn rerun_error(error: impl std::fmt::Display) -> WildDatumError {
    WildDatumError::Internal(format!("Rerun recording error: {error}"))
}

#[cfg(test)]
mod tests {
    use wilddatum_core::DatasetId;
