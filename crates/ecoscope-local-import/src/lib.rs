...
use ecoscope_core::{
    AssetId, AxisRole, Checksum, CubeAxis, CubeDescriptor, EcoScopeError, LocalAssetInspection,
    Modality, Result,
};
use serde_json::json;
...
pub async fn inspect_path(path: &Path) -> Result<LocalAssetInspection> {
    ...
    let (fingerprint, size_bytes) =
        tokio::task::spawn_blocking(move || fingerprint_path_details(&path_for_hash))
            .await
            .map_err(|error| {
                EcoScopeError::Internal(format!("fingerprint task failed: {error}"))
            })??;
...
fn fingerprint_path_details(path: &Path) -> Result<(Checksum, u64)> {
    ...
    if path.is_file() && extension(path) == "shp" {
        hasher.update(b"ecoscope-shapefile-set-v1\0");
        ...
        hasher.update(b"ecoscope-zarr-directory-v1\0");
        let mut files = walkdir::WalkDir::new(path)
            .follow_links(false)
            .into_iter()
            .map(|entry| {
                entry.map_err(|error| EcoScopeError::Invalid(format!("invalid Zarr tree: {error}")))
            })
        ...
        for entry in files {
            ...
            if entry.file_type().is_file() {
                let relative = entry.path().strip_prefix(path).map_err(|error| {
                    EcoScopeError::Internal(format!("cannot fingerprint Zarr path: {error}"))
                })?;
...
fn inspect_image(path: &Path, inspection: &mut LocalAssetInspection) -> Result<()> {
    let reader = image::ImageReader::open(path)
        .map_err(EcoScopeError::from)?
        .with_guessed_format()
        .map_err(EcoScopeError::from)?;
    let (width, height) = reader.into_dimensions().map_err(|error| {
        EcoScopeError::Invalid(format!("cannot read image dimensions: {error}"))
    })?;
...
fn inspect_parquet(path: &Path, inspection: &mut LocalAssetInspection) -> Result<()> {
    ...
    let reader = SerializedFileReader::new(File::open(path)?).map_err(|error| {
        EcoScopeError::Invalid(format!("cannot read Parquet metadata: {error}"))
    })?;
...
fn inspect_geotiff(path: &Path, inspection: &mut LocalAssetInspection) -> Result<()> {
    let raster = geotiff_reader::GeoTiffFile::open(path)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot read GeoTIFF: {error}")))?;
...
fn inspect_lidar(path: &Path, inspection: &mut LocalAssetInspection) -> Result<()> {
    let reader = las::Reader::from_path(path)
        .map_err(|error| EcoScopeError::Invalid(format!("cannot read LAS/LAZ header: {error}")))?;
    let header = reader.header();
...
fn inspect_zarr(path: &Path, inspection: &mut LocalAssetInspection) -> Result<()> {
    ...
    if let Ok(array) = Array::open(store.clone(), "/") {
        ...
        let group = Group::open(store.clone(), "/").map_err(|error| {
            EcoScopeError::Invalid(format!("cannot open Zarr hierarchy: {error}"))
        })?;
        for (node_path, metadata) in group.traverse().map_err(|error| {
            EcoScopeError::Invalid(format!("cannot inspect Zarr hierarchy: {error}"))
        })? {
            if let NodeMetadata::Array(metadata) = metadata {
                arrays.push(
                    Array::new_with_metadata(store.clone(), node_path.as_str(), metadata).map_err(
                        |error| EcoScopeError::Invalid(format!("invalid Zarr array: {error}")),
                    )?,
                );
            }
        }
...
pub fn inspect_hdf5_structure(path: &Path) -> Result<Hdf5Structure> {
    ...
    let root = file
        .group("/")
        .map_err(|error| EcoScopeError::Invalid(format!("cannot read HDF5 root: {error}")))?;
...
fn collect_hdf5_datasets(
    ...
    for dataset in group
        .datasets()
        .map_err(|error| EcoScopeError::Invalid(format!("cannot list HDF5 datasets: {error}")))?
    {
    ...
    for child in group
        .groups()
        .map_err(|error| EcoScopeError::Invalid(format!("cannot list HDF5 groups: {error}")))?
    {
...
fn inspect_delimited(
    ...
    let mut reader = builder
        .from_path(path)
        .map_err(|error| EcoScopeError::Invalid(format!("invalid delimited file: {error}")))?;
    inspection.fields = reader
        .headers()
        .map_err(|error| EcoScopeError::Invalid(format!("cannot read table header: {error}")))?
        .iter()
        .map(str::to_owned)
        .collect();
