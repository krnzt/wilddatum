...
use ecoscope_core::{AggregateSpec, EcoScopeError, QueryFilter, Result, SortSpec};
...
pub async fn execute_geoparquet_region(
    ...
    let path_text = path
        .to_str()
        .ok_or_else(|| EcoScopeError::Invalid("GeoParquet path is not valid UTF-8".into()))?;
    ...
    projection.push(format!(
        "ST_AsText({}) AS __ecoscope_geometry_wkt",
        quote_identifier(geometry_name)
    ));
...
pub fn write_arrow_ipc(path: &Path, batches: &[RecordBatch]) -> Result<()> {
    let schema = batches
        ...
        .ok_or_else(|| EcoScopeError::Invalid("cannot write an empty Arrow result".into()))?;
    let mut writer = FileWriter::try_new(File::create(path)?, &schema)
        .map_err(|error| EcoScopeError::Internal(format!("cannot create Arrow IPC: {error}")))?;
    for batch in batches {
        writer
            .write(batch)
            .map_err(|error| EcoScopeError::Internal(format!("cannot write Arrow IPC: {error}")))?;
    }
    writer
        .finish()
        .map_err(|error| EcoScopeError::Internal(format!("cannot finish Arrow IPC: {error}")))
}

async fn register_source(context: &SessionContext, path: &Path, original_name: &str) -> Result<()> {
    let path_text = path
        .to_str()
        .ok_or_else(|| EcoScopeError::Invalid("table path is not valid UTF-8".into()))?;
    let extension = Path::new(original_name)
        .extension()
        .and_then(|value| value.to_str())
        .unwrap_or_default()
    ...
    match extension.as_str() {
        ...
        "arrow" | "ipc" | "feather" => {
            let reader = FileReader::try_new(File::open(path)?, None).map_err(|error| {
                EcoScopeError::Invalid(format!("cannot read Arrow IPC file: {error}"))
            })?;
            let schema = reader.schema();
            let batches = reader
                .collect::<std::result::Result<Vec<_>, _>>()
                .map_err(|error| EcoScopeError::Invalid(format!("invalid Arrow IPC: {error}")))?;
        ...
        let predicate = match op.as_str() {
            ...
            "in" => {
                let values = filter.value.as_array().ok_or_else(|| {
                    EcoScopeError::Invalid("an in filter requires an array value".into())
                })?;
                if values.is_empty() {
                    "FALSE".into()
                } else {
...
fn batches_to_json(batches: &[RecordBatch]) -> Result<Vec<Map<String, Value>>> {
    ...
    writer.write_batches(&references).map_err(|error| {
        EcoScopeError::Internal(format!("cannot serialize Arrow result: {error}"))
    })?;
    writer.finish().map_err(|error| {
        EcoScopeError::Internal(format!("cannot finish Arrow JSON result: {error}"))
    })?;
    serde_json::from_slice(&writer.into_inner()).map_err(EcoScopeError::from)
}
...
fn extract_count(batches: &[RecordBatch]) -> Result<u64> {
    ...
    rows.first()
        ...
        .ok_or_else(|| EcoScopeError::Internal("DataFusion count returned no value".into()))
}
...
fn query_error(error: impl std::fmt::Display) -> EcoScopeError {
    EcoScopeError::Invalid(format!("tabular query failed: {error}"))
}

#[cfg(test)]
mod tests {
    use super::*;
    use ecoscope_core::SortSpec;
    use std::io::Write;
